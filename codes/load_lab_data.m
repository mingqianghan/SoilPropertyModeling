function [mag, phs, gt] = load_lab_data(mainpath, lab_exptype, ...
...
% Create the full path to the ground truth (GT) file using the provided subpath.
gt_path = fullfile(mainpath, gt_subpath);

% Read the ground truth data from an Excel sheet corresponding to the given experiment number.
gt_data = readtable(gt_path, 'VariableNamingRule', 'preserve', ...
                    'Sheet', lab_expnum);

% Find the rows in the GT data that match the specified cable type ('LC' or 'SC').
positions = find(strcmp(gt_data.("Cable Type"), lab_expcbtype));

% If the experiment type is 'WC' (Water Content):
if lab_exptype == "WC"
    % Extract water content data (calculated, prepared, and sensor readings).
    ...
    % Convert to VWC 
    gt.WC_Calculated = gt_data.("WC_Calculated (g_g)")(positions, :) * bulk_density;
    gt.WC_Prepared = gt_data.("WC_Prepared (g_g)")(positions, :) * bulk_density;
else
    % If the experiment type is 'Nitrogen', extract the nitrogen-related data.
...
% Pre-allocate matrices to store the magnitude and phase data (for all samples and frequencies).
mag = zeros(num_samples, fre_size);
phs = zeros(num_samples, fre_size);

% Loop through each sample to read the corresponding magnitude and phase data.
