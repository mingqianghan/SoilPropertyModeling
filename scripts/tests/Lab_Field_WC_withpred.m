...
results_file_path = 'results\Lab_Field_WC_withpred';
...
for j = 1:length(Predictors)
    ...
    [data_x_W_exp, data_y_W_exp, ~] = extract_and_clean_data(lab_data_W_exp, 'lab', Predictors{j}, 'WC_Calculated');
    [data_x_N_exp, data_y_N_exp, ~] = extract_and_clean_data(lab_data_N_exp, 'lab', Predictors{j}, 'WC');
    ...
    for i = 1: length(plot_name)
        for k = 1:length(N_type)
            ...
            [data_x_field, data_y_field, ~] = extract_and_clean_data(field_data(matches), 'field', Predictors{j}, 'VWC');

            
            [val_x, val_y, test_x, test_y] = train_val_split(data_x_field, data_y_field, 'ratio', 'train_ratio', valid_test_ratio);
