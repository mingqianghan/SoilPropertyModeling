...
for i = 1:length(all_data)
    ...
    if current_sample_num > 0
        ...
        legend_values = arrayfun(@(VWC, NO3, NH4) ...
                        sprintf('%2.1f - %2.1f - %2.1f', VWC, NO3, NH4),...
                        all_data(i).gt.VWC(plot_idx)*100, ...
                        all_data(i).gt.NO3(plot_idx), ...
                        all_data(i).gt.NH4(plot_idx), ...
                        'UniformOutput', false);

        % Plot magnitude
        plot_data(fre, all_data(i).data.mag, plot_idx, ...
                  [all_data(i).Plotname '-' all_data(i).Ntype '-' ...
                   all_data(i).Cabletype '-' 'Mag'], ...
                  legend_values, ' V_{probe}/V_{total} (dB)', ...
                  fontsize, colors);

        % Plot phase
        plot_data(fre, all_data(i).data.phs, plot_idx, ...
                  [all_data(i).Plotname '-' all_data(i).Ntype '-' ...
                   all_data(i).Cabletype '-' 'Phs'], ...
                  legend_values, 'Absolute Phase Difference (degrees)', ...
                  fontsize, colors);
    end
end
...
function plot_data(frequencies, data, plot_idx, title_str, ...
                   legend_values, ylabel_str, fontsize, colors)
    figure;
    % Adjust size [left, bottom, width, height]
    set(gcf, 'Position', [100, 100, 850, 600]); 
    hold on;
    for j = 1:length(plot_idx)
        semilogx(frequencies, data(plot_idx(j),:), 'LineWidth', 2, ...
                 'Color', colors(j, :));
    end
